...
describe('UserRouter', () => {
  ...
  // Test add user
  describe(`"POST:${Add}"`, () => {
    const callApi = (reqBody: TReqBody) => agent.post(Add).type('form').send(reqBody);

    // Test add user success
    it(
      `should return a status code of "${CREATED}" if the request was ` + 'successful.',
      (done) => {
        // Spy
  ...
  // ** Update users ** //
  describe(`"PUT:${Update}"`, () => {
    const callApi = (reqBody: TReqBody) => agent.put(Update).type('form').send(reqBody);
  ...
  describe(`"DELETE:${Delete}"`, () => {
    const callApi = (id: number) => agent.delete(insertUrlParams(Delete, { id }));
